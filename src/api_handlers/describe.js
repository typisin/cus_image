import { logUsage } from '../../lib/tursoClient.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      try { await logUsage('/api/describe', { upstream_status: r.status, error: text || 'Upstream error' }) } catch (_) {}
    ...
    try {
      await logUsage('/api/describe', { detail, length: description.length })
    } catch (_) {}
    res.status(200).json({ description })
  } catch (e) {
    try { await logUsage('/api/describe', { error: e && e.message }) } catch (_) {}
